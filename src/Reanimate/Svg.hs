...
lowerTransformations = worker False Transform.identity
  where
    ...
    worker hasPathified m t =
        let m' = m * Transform.mkMatrix (t ^. transform)
         in case t of
                PathTree path ->
                    PathTree $
                        path
                            & pathDefinition %~ updPath m'
                            & transform .~ Nothing
                GroupTree g ->
                    GroupTree $
                        g
                            & groupChildren %~ map (worker hasPathified m')
                            & transform .~ Nothing
                LineTree line ->
                    LineTree $
                        line
                            & linePoint1 %~ updPoint m
  ...
  where
    ...
    worker (GroupTree g)
        ...
        | otherwise =
            dropNulls $
                GroupTree $
                    g & groupChildren %~ concatMap worker
  ...
  where
    worker =
        \case
            ...
            LineTree line
                | Just (x1, y1, x2, y2) <- unpackLine line ->
                    PathTree $
                        defaultSvg
            ...
            EllipseTree elip
                | Just (cx, cy, rx, ry) <- unpackEllipse elip ->
                    PathTree $
                        defaultSvg
    ...
    unpackEllipse elip = do
        ...
        liftM4
            ...
            (unpackNumber x)
            (unpackNumber y)
            (unpackNumber $ elip ^. ellipseXRadius)
